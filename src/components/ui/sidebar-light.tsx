...
interface SidebarLightProps {
  items: NavItem[]
  className?: string
  /** When provided, overrides pathname-based active detection. */
  activeHref?: string
}
...
function NavItemRenderer({ item, pathname, activeHref, depth }: NavItemRendererProps) {
  const hasChildren = item.items && item.items.length > 0
  const isActive = activeHref ? item.href === activeHref : pathname === item.href
...
function SidebarLight({ items, className, activeHref }: SidebarLightProps) {
