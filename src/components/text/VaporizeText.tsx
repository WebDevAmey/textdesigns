...
type VaporizeTextProps = {
  /** Single text (library default) — used when `texts` is not provided. */
  text?: string;
  texts?: string[];
  font?: {
    fontFamily?: string;
...
export default function VaporizeText({
      ...
      switch (animationState) {
        ...
        case "vaporizing": {
          ...
          if (vaporizeProgressRef.current >= 100 && allVaporized) {
            setCurrentTextIndex(
              (prevIndex) => (prevIndex + 1) % allTexts.length
            );
  ...
  useEffect(() => {
    if (wrapperRef.current) {
      ...
      setWrapperSize({
        width: rect.width || width || 0,
        height: rect.height || height || 0,
      });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  return (
    <div ref={wrapperRef} style={wrapperStyle}>
      ...
      <SeoElement tag={tag} texts={allTexts} />
    </div>
  );
}
...
const renderCanvas = ({
  ...
  const fontSize = parseInt(
    framerProps.font?.fontSize?.replace("px", "") || "70"
  );
  const font = `${framerProps.font?.fontWeight ?? 600} ${
    fontSize * globalDpr
  ...
  const currentText =
    framerProps.texts?.[currentTextIndex] || framerProps.text || "Vaporize";
